...
public sealed class ImmovableRodSystem : EntitySystem
{
    ...
    private void OnCollide(EntityUid uid, ImmovableRodComponent component, ref StartCollideEvent args)
    {
        ...
        if (HasComp<ImmovableRodComponent>(ent))
        {
            ...
            _adminLogger.AddStructured(LogType.Gib, LogImpact.Low, $"{uid} and {ent} created singularity at X:{coords.X} Y:{coords.Y}",
                entities: new[]
                {
                    new AdminLogEntityRef(uid, AdminLogEntityRole.Actor),
                    new AdminLogEntityRef(ent, AdminLogEntityRole.Target)
                });
        ...
        if (HasComp<BodyComponent>(ent))
        {
            ...
            var coords = Transform(uid).Coordinates;
            _adminLogger.AddStructured(LogType.Gib, LogImpact.Low, $"Entity {uid} gibbed {ent} at X:{coords.X} Y:{coords.Y}",
                entities: new[]
                {
                    new AdminLogEntityRef(uid, AdminLogEntityRole.Actor),
                    new AdminLogEntityRef(ent, AdminLogEntityRole.Victim)
                });
