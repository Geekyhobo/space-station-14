...
public sealed class TelephoneSystem : SharedTelephoneSystem
{
    ...
    private void SendTelephoneMessage(EntityUid messageSource, string message, Entity<TelephoneComponent> source, bool escapeMarkup = true)
    {
        ...
        if (name != Name(messageSource))
            _adminLogger.AddStructured(LogType.Chat, LogImpact.Low, $"Telephone message from {messageSource:user} as {name} on {source}: {message}");
        else
            _adminLogger.AddStructured(LogType.Chat, LogImpact.Low, $"Telephone message from {messageSource:user} on {source}: {message}");
