    ...
    public sealed partial class VoteManager
    {
        ...
        public void CreateStandardVote(ICommonSession? initiator, StandardVoteType voteType, string[]? args = null)
        {
            if (initiator != null && args == null)
                _adminLogger.AddStructured(LogType.Vote, LogImpact.Medium, $"{initiator} initiated a {voteType.ToString()} vote");
            else if (initiator != null && args != null)
                _adminLogger.AddStructured(LogType.Vote, LogImpact.Extreme, $"{initiator} initiated a {voteType.ToString()} vote with the arguments: {String.Join(",", args)}");
            else
                _adminLogger.AddStructured(LogType.Vote, LogImpact.Medium, $"Initiated a {voteType.ToString()} vote");
        ...
        private void StartVote(ICommonSession? initiator)
        {
            ...
            vote.OnFinished += (_, _) =>
            {
                ...
                if (total > 0 && votesYes / (float) total >= ratioRequired)
                {
                    ...
                    {
                        _adminLogger.AddStructured(LogType.Vote, LogImpact.Medium, $"Restart vote attempted to pass, but an admin was online. {votesYes}/{votesNo}");
                    }
                    else // If the cvar is disabled or there's no admins on, proceed as normal
                    {
                        _adminLogger.AddStructured(LogType.Vote, LogImpact.Medium, $"Restart vote succeeded: {votesYes}/{votesNo}");
                ...
                {
                    _adminLogger.AddStructured(LogType.Vote, LogImpact.Medium, $"Restart vote failed: {votesYes}/{votesNo}");
        ...
        private void NotifyNotEnoughGhostPlayers(int ghostPercentageRequirement, int roundedGhostPercentage)
        {
            ...
            _adminLogger.AddStructured(LogType.Vote, LogImpact.Medium, $"Restart vote failed: Current Ghost player percentage:{roundedGhostPercentage.ToString()}% does not meet {ghostPercentageRequirement.ToString()}%");
            _chatManager.DispatchServerAnnouncement(
        ...
        private void CreatePresetVote(ICommonSession? initiator)
        {
            ...
            vote.OnFinished += (_, args) =>
            {
                ...
                _adminLogger.AddStructured(LogType.Vote, LogImpact.Medium, $"Preset vote finished: {picked}");
        ...
        private void CreateMapVote(ICommonSession? initiator)
        {
            ...
            vote.OnFinished += (_, args) =>
            {
                ...
                _adminLogger.AddStructured(LogType.Vote, LogImpact.Medium, $"Map vote finished: {picked.MapName}");
                var ticker = _entityManager.EntitySysManager.GetEntitySystem<GameTicker>();
        ...
        private async void CreateVotekickVote(ICommonSession? initiator, string[]? args)
        {
            ...
            if (_votingSystem != null && !await _votingSystem.CheckVotekickInitEligibility(initiator))
            {
                ...
                _adminLogger.AddStructured(LogType.Vote, LogImpact.Extreme, $"Votekick attempted by {initiator}, but they are not eligible to votekick!");
                DirtyCanCallVoteAll();
                return;
            }
            ...
            if (located == null)
            {
                ...
                _adminLogger.AddStructured(LogType.Vote, LogImpact.Extreme, $"Votekick attempted by {initiator} for player string {target}, but they could not be found!");
                DirtyCanCallVoteAll();
                return;
            }
            ...
            {
                _adminLogger.AddStructured(LogType.Vote, LogImpact.Extreme, $"Votekick attempted by {initiator} for themselves? Votekick cancelled.");
                DirtyCanCallVoteAll();
                return;
            }
            ...
            {
                _adminLogger.AddStructured(LogType.Vote, LogImpact.Extreme, $"Votekick attempted by {initiator} for player {targetSession}, but there were not enough ghost roles! {eligibleVoterNumberRequirement} required, {eligibleVoterNumber} found.");
                if (initiator != null)
                {
            ...
            {
                _adminLogger.AddStructured(LogType.Vote, LogImpact.Extreme, $"Votekick attempted by {initiator} for player {targetSession}, but they are not eligible to be votekicked!");
                DirtyCanCallVoteAll();
                return;
            }
            ...
            var vote = CreateVote(options);
            _adminLogger.AddStructured(LogType.Vote, LogImpact.Extreme, $"Votekick for {located.Username} ({targetEntityName}) due to {reason} started, initiated by {initiator}.");
            ...
            vote.OnFinished += (_, eventArgs) =>
            {
                ...
                if (total > 0 && votesYes / (float)total >= ratioRequired)
                {
                    ...
                    {
                        _adminLogger.AddStructured(LogType.Vote, LogImpact.Extreme, $"Votekick for {located.Username} attempted to pass, but an admin was online. Yes: {votesYes} / No: {votesNo}. Yes: {yesVotersString} / No: {noVotersString}");
                    ...
                    {
                        _adminLogger.AddStructured(LogType.Vote, LogImpact.Extreme, $"Votekick for {located.Username} due to {reason} finished, created by {initiator}, but was cancelled due to the target being an antagonist.");
                        AnnounceCancelledVotekickForVoters(targetEntityName);
                    ...
                    {
                        _adminLogger.AddStructured(LogType.Vote, LogImpact.Extreme, $"Votekick for {located.Username} due to {reason} finished, created by {initiator}, but was cancelled due to the target being a de-admined admin.");
                        AnnounceCancelledVotekickForVoters(targetEntityName);
                    ...
                    {
                        _adminLogger.AddStructured(LogType.Vote, LogImpact.Extreme, $"Votekick for {located.Username} succeeded:  Yes: {votesYes} / No: {votesNo}. Yes: {yesVotersString} / No: {noVotersString}");
                ...
                else
                {
                    ...
                    _adminLogger.AddStructured(LogType.Vote, LogImpact.Extreme, $"Votekick failed: Yes: {votesYes} / No: {votesNo}. Yes: {yesVotersString} / No: {noVotersString}");
