    ...
    internal sealed class PointingSystem : SharedPointingSystem
    {
        ...
        public bool TryPoint(ICommonSession? session, EntityCoordinates coordsPointed, EntityUid pointed)
        {
            ...
            if (Exists(pointed))
            {
                ...
                _adminLogger.AddStructured(LogType.Action, LogImpact.Low, $"{player:user} pointed at {pointed:target} {Transform(pointed).Coordinates}");
            }
            else
            {
                ...
                _adminLogger.AddStructured(LogType.Action, LogImpact.Low, $"{player:user} pointed at {name} {(position == null ? mapCoordsPointed : position)}");
            }
