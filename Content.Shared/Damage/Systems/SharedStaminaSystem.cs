...
public abstract partial class SharedStaminaSystem : EntitySystem
{
        ...
        EntityUid? source = null, EntityUid? with = null, bool visual = true, SoundSpecifier? sound = null, bool ignoreResist = false)
    {
        ...
        if (source != null)
        {
            _adminLogger.AddStructured(LogType.Stamina, $"{source.Value:user} caused {value} stamina damage to {uid:target}{(with != null ? $" using {with.Value:using}" : "")}");
        }
        else
        {
            _adminLogger.AddStructured(LogType.Stamina, $"{uid:target} took {value} stamina damage");
        }
    ...
    private void EnterStamCrit(EntityUid uid, StaminaComponent? component = null)
    {
        ...
        EnsureComp<ActiveStaminaComponent>(uid);
        Dirty(uid, component);
        _adminLogger.AddStructured(LogType.Stamina, LogImpact.Medium, $"{uid:user} entered stamina crit");
    }
    ...
    private void ExitStamCrit(EntityUid uid, StaminaComponent? component = null)
    {
        ...
        UpdateStaminaVisuals((uid, component));
        Dirty(uid, component);
        _adminLogger.AddStructured(LogType.Stamina, LogImpact.Low, $"{uid:user} recovered from stamina crit");
    }
